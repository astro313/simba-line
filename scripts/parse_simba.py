...
def save_SFgal_txt(galnames, outFilePrefix):
    """

    Save output from select_SFgal_from_simba() into .txt files that are 1) sorted by SFR, and another sorted by snapshot number
    ...
    ----------
    galnames:
        contains haloID, galID, SFR, snapshot number of the selected SF galaxies from select_SFgal_from_simba()
    ...
    np.savetxt('SFgal_sortSFR.txt', galnames, header='index\tSFR\thalo number \tsnapshot')

    # now, sort by snapshot number
    ...
    np.savetxt('SFgal_sortsnap.txt', galnames, header='index\tSFR\thalo number \tsnapshot')


def split_by_snap(inFile='SFgal_sortsnap.txt'):

    """
    split out all galnames: GAL, halo, snap, SFR into different .txt, grouped by snapshot number, for multiprocessing

    """

    import numpy as np
    bubu = np.loadtxt(inFile)
    snap = map(int, bubu[:, 3])
    uniqueSnap = np.unique(snap)

    for iii in uniqueSnap:
        idx = np.where(snap == iii)
        np.savetxt(inFile.replace('.txt', '_' + str(iii) + '.txt'), bubu[idx])
...
def process(line):
    ...
    """
    extract simba galaxies into pd dataframe using multiprocessing
    """
    ...
    # where the pickle files are stored
    ...
    if not os.path.exists(simgas_path) and not os.path.exists(simstar_path) and not os.path.exists(simdm_path):
        # only run below if we didn't extract these galaxies before..
        ...
        # sphere = ds.sphere(loc, R_gal)      # bubu
...
def simba_to_pd_multiprocessing(nproc, raw_sim_dir, raw_sim_name_prefix, caesar_dir, name_prefix, redshiftFile, dd_data, snap=36, fNamePrefix='SFgal_sortsnap_'):

    '''
       Use multiprocessing to write out useful fields gas, stars, DM particles of the selected galaxies into DataFrames
   ...
   Parameters
    ...
    # define global ds and obj
    global zred
    # zred = define_ds(snap, raw_sim_dir, raw_sim_name_prefix, caesar_dir, name_prefix, redshiftFile)
    zred = 1    # bubu

    def_global_d_data(dd_data)
    global d_data

    fName = fNamePrefix + str(snap) + '.txt'
    lines = np.loadtxt(fName)
    zreds_selected = np.ones(len(lines)) * zred
    results = pool.map(process, lines)
    pool.close()
    return results, zreds_selected
...
def simba_to_pd(galnames, raw_sim_dir, raw_sim_name_prefix, caesar_dir, name_prefix, redshiftFile, d_data, verbose=False, debug=False):
    ...
    will be removed in future, replaced by simba_to_pd_multiprocessing()
